...
use crate::{auth::User, db_structure::{EZTable, DbVec, KeyString, Metadata, Value}, networking_utilities::*, server_networking::{Server, HeavyThreadMessage, CONFIG_FOLDER}};
...
pub fn handle_download_request(
    ...
    disk_thread_sender: crossbeam_channel::Sender<HeavyThreadMessage>
) -> Result<(), ServerError> {
    ...
    if response == "OK" {
        ...
        match disk_thread_sender.try_send(HeavyThreadMessage::UpdateMetadata(metadelta, KeyString::from(name))) {
            Ok(_) => {},
            Err(e) => match e {
...
pub fn handle_upload_request(
    ...
    match EZTable::from_csv_string(bytes_to_str(&csv)?, name, "test") {
        Ok(table) => {
            ...
            match disk_thread_sender.try_send(HeavyThreadMessage::NewTable(table)) {
                Ok(_) => {},
                Err(e) => match e {
                    crossbeam_channel::TrySendError::Disconnected(_e) => panic!("write thread has closed. Server is dead!!!"),
                    crossbeam_channel::TrySendError::Full(_e) => todo!("I don't exactly know what to do here yet"),
...
pub fn handle_update_request(connection: &mut Connection, name: &str, global_tables: Arc<RwLock<HashMap<KeyString, RwLock<EZTable>>>>, disk_thread_sender: crossbeam_channel::Sender<HeavyThreadMessage>) -> Result<String, ServerError> {
    ...
    match EZTable::from_csv_string(csv, "insert", "system") {
        Ok(table) => {
            match disk_thread_sender.try_send(HeavyThreadMessage::UpdateTable(KeyString::from(name), table)) {
                Ok(_) => {
...
pub fn handle_query_request(connection: &mut Connection, name: &str, query: &str, global_tables: Arc<RwLock<HashMap<KeyString, RwLock<EZTable>>>>, disk_thread_sender: crossbeam_channel::Sender<HeavyThreadMessage>) -> Result<String, ServerError> {
    match connection.stream.write("OK".as_bytes()) {
        Ok(n) => println!("Wrote {n} bytes"),
        Err(e) => {return Err(ServerError::Io(e.kind()));},
    };
...
/// This will be rewritten to use EZQL soon.
pub fn handle_delete_request(connection: &mut Connection, name: &str, query: &str, global_tables: Arc<RwLock<HashMap<KeyString, RwLock<EZTable>>>>, disk_thread_sender: crossbeam_channel::Sender<HeavyThreadMessage>) -> Result<String, ServerError> {
    match connection.stream.write("OK".as_bytes()) {
        Ok(n) => println!("Wrote {n} bytes"),
        Err(e) => {return Err(ServerError::Io(e.kind()));},
    };
...
pub fn handle_new_user_request(connection: &mut Connection, user_string: &str, users: Arc<RwLock<HashMap<KeyString, RwLock<User>>>>, disk_thread_sender: crossbeam_channel::Sender<HeavyThreadMessage>) -> Result<(), ServerError> {

    let user: User = ron::from_str(user_string).unwrap();

    match disk_thread_sender.try_send(HeavyThreadMessage::MetaNewUser(user)) {
        Ok(_) => {
            connection.stream.write_all("OK".as_bytes())?;
        },
        Err(e) => match e {
...
pub fn handle_kv_upload(connection: &mut Connection, key: &str, disk_thread_sender: crossbeam_channel::Sender<HeavyThreadMessage>) -> Result<(), ServerError> {
    ...
    println!("Appending to global");
    match disk_thread_sender.try_send(HeavyThreadMessage::NewKeyValue(KeyString::from(key), value)) {
        Ok(_) => {
            connection.stream.write_all("OK".as_bytes())?;
        },
        Err(e) => match e {
...
pub fn handle_kv_update(connection: &mut Connection, key: &str, global_kv_table: Arc<RwLock<HashMap<KeyString, RwLock<Value>>>>, disk_thread_sender: crossbeam_channel::Sender<HeavyThreadMessage>) -> Result<(), ServerError> {
...
pub fn handle_kv_download(connection: &mut Connection, name: &str, global_kv_table: Arc<RwLock<HashMap<KeyString, RwLock<Value>>>>, disk_thread_sender: crossbeam_channel::Sender<HeavyThreadMessage>) -> Result<(), ServerError> {
...
pub fn handle_meta_list_tables(connection: &mut Connection, global_tables: Arc<RwLock<HashMap<KeyString, RwLock<EZTable>>>>, disk_thread_sender: crossbeam_channel::Sender<HeavyThreadMessage>) -> Result<(), ServerError> {
...
pub fn handle_meta_list_key_values(connection: &mut Connection, global_kv_table: Arc<RwLock<HashMap<KeyString, RwLock<Value>>>>, disk_thread_sender: crossbeam_channel::Sender<HeavyThreadMessage>) -> Result<(), ServerError> {

...
mod tests {
    ...
    fn test_bloom_filter() {
        ...
        let rec_size = Bloom::<i32>::compute_bitmap_size(test_data.len(), 0.001);

        println!("rec_size: {}", rec_size);

        let mut bloom = Bloom::<i32>::new_with_seed(rec_size, test_data.len(), &rand::random::<[u8;32]>());

        for item in test_data {
            bloom.set(&item);
        }

        let check = bloom.check(&23);
